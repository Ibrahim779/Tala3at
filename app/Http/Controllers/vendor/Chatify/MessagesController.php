...
class MessagesController extends Controller
{
    ...
    public function idFetchData(Request $request)
    {
        ...
        if ($request['type'] == 'user') {
            $fetch = Admin::where('id', $request['id'])->first();
        }
        ...
        if ($users->count() > 0) {
            ...
            foreach ($users as $user) {
                if ($user->id != Auth::user()->id) {
                    // Get user data
                    $userCollection = Admin::where('id', $user->id)->first();
    ...
    {
        // Get user data
        $userCollection = Admin::where('id', $request['user_id'])->first();
    ...
    public function getFavorites(Request $request)
    {
        ...
        foreach ($favorites->get() as $favorite) {
            // get user data
            $user = Admin::where('id', $favorite->favorite_id)->first();
    ...
    public function search(Request $request)
    {
        ...
        $records = Admin::where('name', 'LIKE', "%{$input}%");
    ...
    public function setActiveStatus(Request $request)
    {
        $update = $request['status'] > 0
            ? Admin::where('id', $request['user_id'])->update(['active_status' => 1])
            : Admin::where('id', $request['user_id'])->update(['active_status' => 0]);
        // send the response
        return Response::json([
            'status' => $update,
        ], 200);
